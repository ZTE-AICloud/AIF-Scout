...
SHELL_SCRIPT = "check_nic_pcie_link_state.sh"
...
class CheckNicPCIeLinkStateTask(Task):
    PSSH_EXEC_TIMEOUT = 120
    sync_check_node = True

    metadata = TaskMetadata(
        check_item="nic_pcie_link_state_check",
    )

    def execute(self) -> None:
        try:
            self.save_check_item_progress(
                TaskStatus.INPROGRESS.value, "Start exec check nic pcie link state...")
            logger.info("Start exec check nic pcie link state...")
            ...
            logger.error(f"Check nic pcie link state exception: {e}")
