...
def extract_data(gpu_manufacturer, stdout):
    """
    stdout:
        ["host -> gpu 0: 24.89", "host -> gpu 1: 24.91", ...]
    """
    return_data = {}
    for line in stdout:
        gpu_matches = re.search(r"host\s*->\s*gpu\s*(\d+):\s*(\d+\.\d+)", line)
        if not gpu_matches:
            continue
        gpu_info = gpu_matches.groups()
        return_data.update({"gpu{}".format(gpu_info[0]): float(gpu_info[1])})
    return return_data


class CheckGpuHToDTask(Task):
    PSSH_EXEC_TIMEOUT = 120
    sync_check_node = True

    metadata = TaskMetadata(
        check_item="gpu_h2d_check",
    )

    def execute(self) -> None:
        try:
            self.save_check_item_progress(
                TaskStatus.INPROGRESS.value, "Start exec check gpu h2d..."
            )
            logger.info("Start exec check gpu h2d...")
            ...
            logger.error(f"Check gpu h2d exception: {e}")
