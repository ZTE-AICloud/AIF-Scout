...
def parse_gpu_driver(line: str) -> str:
    driver = ""
    ret = line.split(SPLIT_GPU_DRIVER)
    if len(ret) >= 1:
        driver = ret[1].strip()
    logger.info("gpu driver: %s", driver)
    return driver


class CheckGpuDriverTask(Task):
    PSSH_EXEC_TIMEOUT = 120
    sync_check_node = True

    metadata = TaskMetadata(
        check_item="gpu_driver_check",
    )

    def execute(self) -> None:
        try:
            self.save_check_item_progress(
                TaskStatus.INPROGRESS.value, "Start exec check gpu driver..."
            )
            logger.info("Start exec check gpu driver...")
            ...
            logger.error(f"Check gpu driver exception: {e}")

    def _handle_result(self, host_output, node_ids):
        gpu_driver = self.task_options.task_params.get("gpu_driver")
        abnormal_host = []
        for o in host_output:
            ...
            check_gpu_driver = ""
            is_gpu_driver_ok = False
            for line in stdout:
                detail_result.append(extract_data(line))
                if SPLIT_GPU_DRIVER in line:
                    check_gpu_driver = parse_gpu_driver(line)
            if check_gpu_driver == gpu_driver:
                is_gpu_driver_ok = True
            format_result["data"].append(
                [
                    "GPU Driver",
                    check_gpu_driver,
                    gpu_driver,
                    (TaskResult.NORMAL.value if is_gpu_driver_ok else TaskResult.ABNORMAL.value),
                ]
            )
            status = TaskStatus.SUCCESS.value if is_gpu_driver_ok else TaskStatus.FAILED.value
            if status == TaskStatus.FAILED.value:
                abnormal_host.append(o.host)
            detail_result.append(o.host + " exec " + status)
            detail_result.append(
                f"GPU driver {'normal' if is_gpu_driver_ok else 'abnormal'}")
