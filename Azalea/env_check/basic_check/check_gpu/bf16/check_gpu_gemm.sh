...
function check_nvidia_gpu_gemm() {
    case "${input_data_type}" in
        "FP32"|"TF32"|"INT8"|"BF16")
            ;;
        *)
            echo "Error: unsupported data type $input_data_type" >&2
            exit 1
            ;;
    esac
    ...
    sudo docker exec $docker_name bash -lc "sh /workspace/tools/tflops.sh $input_data_type"
}

cd $SCRIPT_PATH
if [[ "$input_manufacturer" == "NVIDIA" ]]; then
    check_nvidia_gpu_gemm
else
