...
SHELL_SCRIPT = "check_gpu_membw.sh"
...
def extract_data(gpu_manufacturer, stdout):
    return_data = {}
    if gpu_manufacturer == GpuManufacturer.NVIDIA.value:
        pattern = r'gpu\s+(\d+):\s+([\d\.]+)'
        for line in stdout:
            match = re.match(pattern, line)
            gpu_id = match.group(1)
            gpu_value = match.group(2)
            return_data.update(
                {"gpu{}".format(gpu_id): float(gpu_value)}
            )
    return_data


class CheckGpuMemBWTask(Task):
    PSSH_EXEC_TIMEOUT = 120
    sync_check_node = True

    metadata = TaskMetadata(
        check_item="gpu_membw_check",
    )

    def execute(self) -> None:
        try:
            self.save_check_item_progress(
                TaskStatus.INPROGRESS.value, "Start exec check gpu membw..."
            )
            logger.info("Start exec check gpu membw...")
            ...
            logger.error(f"Check gpu membw exception: {e}")
