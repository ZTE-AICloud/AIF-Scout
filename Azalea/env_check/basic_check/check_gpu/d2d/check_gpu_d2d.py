...
def extract_data(gpu_manufacturer, stdout):
    """
    stdout:
        ["gpu 0 -> gpu 1: 22.51", "gpu 0 -> gpu 2: 22.65", ...]
    """
    return_data = {}
    for line in stdout:
        matches = re.search(
            r"gpu\s*(\d+)\s*->\s*gpu\s*(\d+)\s*:\s*(\d+\.\d+)", line)
        if not matches:
            continue
        gpu_info = matches.groups()
        return_data.update(
            {"gpu ({0} -> {1} )".format(gpu_info[0],
                                        gpu_info[1]): float(gpu_info[2])}
        )
    return return_data


class CheckGpuDToDTask(Task):
    PSSH_EXEC_TIMEOUT = 300
    sync_check_node = True

    metadata = TaskMetadata(
        check_item="gpu_d2d_check",
    )

    def execute(self) -> None:
        try:
            self.save_check_item_progress(
                TaskStatus.INPROGRESS.value, "Start exec check gpu d2d..."
            )
            logger.info("Start exec check gpu d2d...")
            ...
            logger.error(f"Check gpu d2d exception: {e}")
