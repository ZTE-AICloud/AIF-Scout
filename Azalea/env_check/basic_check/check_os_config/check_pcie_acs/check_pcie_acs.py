...
SHELL_SCRIPT = "check_pcie_acs.sh"
...
class CheckPCIeAcsTask(Task):
    PSSH_EXEC_TIMEOUT = 120
    sync_check_node = True

    metadata = TaskMetadata(
        check_item="pcie_acs_check",
    )

    def execute(self) -> None:
        try:
            self.save_check_item_progress(
                TaskStatus.INPROGRESS.value, "Start exec check pcie acs...")
            logger.info("Start exec check pcie acs...")
            ...
            logger.error(f"Check pcie acs exception: {e}")
