...
SHELL_SCRIPT = "check_roce_network_config.sh"
...
def extract_data(raw_data):
    raw_data = raw_data.replace("[InspectorRet Title]", "")
...
class CheckRoceNetworkConfigTask(Task):
    PSSH_EXEC_TIMEOUT = 120
    sync_check_node = True

    metadata = TaskMetadata(
        check_item="roce_network_config_check",
    )

    def execute(self) -> None:
        try:
            self.save_check_item_progress(
                TaskStatus.INPROGRESS.value, "Start exec check roce network config...")
            logger.info("Start exec check roce network config...")
            ...
            host_output: List[HostOutput] = exec_cmd_on_multi_hosts_realtime(
                host_ips,
                host_config,
                f"chmod +x {dest_path} && {dest_path} '{param_nic}' '{gid_index}'",
            ...
            logger.error(f"Check roce network config exception: {e}")
