...
SHELL_SCRIPT = "check_cpu_idle_state.sh"
...
class CheckCPUIdleStateTask(Task):
    PSSH_EXEC_TIMEOUT = 120
    sync_check_node = True

    metadata = TaskMetadata(
        check_item="cpu_idle_state_check",
    )

    def execute(self) -> None:
        try:
            self.save_check_item_progress(
                TaskStatus.INPROGRESS.value, "Start exec check cpu idle state...")
            logger.info("Start exec check cpu idle state...")
            ...
            logger.error(f"Check cpu idle state exception: {e}")
