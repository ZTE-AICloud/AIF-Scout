...
SHELL_SCRIPT = "check_raid_card.sh"
CURRENT_DIR = os.path.dirname(__file__)
LOCAL_PATH = os.path.join(CURRENT_DIR, SHELL_SCRIPT)
DEST_PATH = os.path.join(Task.REMOTE_WORKSPACE, SHELL_SCRIPT)

SPLIT_RAID_CARD_MODEL = "[InspectorRet]Raid card model:"
SPLIT_RAID_CARD_COUNT = "[InspectorRet]Raid card count:"
...
def parse_raid_card_model(line: str) -> str:
    model = ""
    ret = line.split(SPLIT_RAID_CARD_MODEL)
    if len(ret) >= 1:
        model = ret[1].strip()
    logger.info("raid card model: %s", model)
    return model


def parse_raid_card_count(line: str) -> int:
    count = ""
    ret = line.split(SPLIT_RAID_CARD_COUNT)
    if len(ret) >= 1:
        count = ret[1].strip()
    logger.info("raid card count: %s", count)
    if count.isdigit():
        return int(count)
    return 0


class CheckRaidCardTask(Task):
    PSSH_EXEC_TIMEOUT = 120
    sync_check_node = True

    metadata = TaskMetadata(
        check_item="raid_card_check",
    )

    def execute(self) -> None:
        try:
            self.save_check_item_progress(
                TaskStatus.INPROGRESS.value, "Start exec check raid card..."
            )
            logger.info("Start exec check raid card...")
            ...
            logger.error(f"Check raid card exception: {e}")

    def _handle_result(self, host_output, node_ids):
        raid_card_model = self.task_options.task_params.get("raid_card_model")
        raid_card_count = self.task_options.task_params.get("raid_card_count")
        abnormal_host = []
        for o in host_output:
            ...
            check_raid_card_model = ""
            check_raid_card_count = 0
            is_raid_card_model_ok = False
            is_raid_card_count_ok = False
            for line in stdout:
                detail_result.append(extract_data(line))
                if SPLIT_RAID_CARD_MODEL in line:
                    check_raid_card_model = parse_raid_card_model(line)
                elif SPLIT_RAID_CARD_COUNT in line:
                    check_raid_card_count = parse_raid_card_count(line)
            if check_raid_card_model == raid_card_model:
                is_raid_card_model_ok = True
            if check_raid_card_count == raid_card_count:
                is_raid_card_count_ok = True
            ...
            format_result["data"].append(
                [
                    "Raid Card Count",
                    check_raid_card_count,
                    raid_card_count,
                    (
                        TaskResult.NORMAL.value
                        if is_raid_card_count_ok
                        else TaskResult.ABNORMAL.value
                    ),
                ]
            )
            status = (
                ...
                if is_raid_card_model_ok and is_raid_card_count_ok
            ...
            detail_result.append(
                f"Raid card model {'normal' if is_raid_card_model_ok else 'abnormal'}"
            )
            detail_result.append(
                f"Raid card count {'normal' if is_raid_card_count_ok else 'abnormal'}"
            )
