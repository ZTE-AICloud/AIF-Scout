...
function check_roce_network_bandwidth_between_2nodes_same_device
{
    ...
    if [ ! -z "$NETWORK_DEVICES" ]; then
        ...
        do
            # ipv4 ifconfig $mlx_device | sed -n "2p" | grep -w inet | awk '{print $2}'
            # ipv6 ifconfig $mlx_device | sed -n "2p" | grep -w inet6 | awk '{print $2}'
    ...
    do
        ...
        cmd2="$IB_TOOL -d ${client_mlx_all[$i]} -x ${GID} -s ${MESSAGE_SIZE} -q ${QUEUE_SIZE} ${IB_ARGS} -F --report_gbits ${server_eth_ips[$i]}"
        ...
        if [[ $? -ne 0 ]];then
            echo "FAILED! RoCE network bandwidth between Server-${node1}-${server_eth_all[$i]} and Client-${node2}-${client_eth_all[$i]}: Server-${server_mlx_all[$i]}-${server_eth_ips[$i]} Client-${client_mlx_all[$i]}-${client_eth_ips[$i]}"
            # clean
            ssh_exec $server "pkill -9 $IB_TOOL"
            ssh_exec $client "pkill -9 $IB_TOOL"
            continue
