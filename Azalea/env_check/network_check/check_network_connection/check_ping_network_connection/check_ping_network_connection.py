...
SHELL_SCRIPT = "check_ping_network_connection.sh"
...
class CheckPingNetworkConnectionTask(Task):
    PSSH_EXEC_TIMEOUT = 120

    metadata = TaskMetadata(
        check_item="ping_network_connection_check",
    )

    def execute(self) -> None:
        try:
            self.save_check_item_progress(
                TaskStatus.INPROGRESS.value, "Start exec check ping network connection...")
            logger.info("Start exec check ping network connection...")
            ...
            target_addrs = self.task_options.task_params.get("target_addrs")
            packet_size = self.task_options.task_params.get("packet_size")
            packet_count = self.task_options.task_params.get("packet_count")
            egress_interface = self.task_options.task_params.get(
                "egress_interface")

            addr_len = len(target_addrs.split(","))
            interface_len = 1
            if egress_interface is not None:
                interface_len = len(egress_interface.split(","))
            timeout = addr_len * interface_len * \
                packet_count * 10 + self.PSSH_EXEC_TIMEOUT
            ...
            host_output: List[HostOutput] = exec_cmd_on_multi_hosts_realtime(
                host_ips,
                host_config,
                f"chmod +x {dest_path} && {dest_path} '{target_addrs}' '{packet_size}' '{packet_count}' '{egress_interface}'",
                timeout=timeout,
            ...
            logger.error(f"Check ping network connection exception: {e}")
