...
GID=${input_gid:-3} # default 3，修复参数格式
...
server_result_file="/tmp/ib_send_lat_server.txt"
client_result_file="/tmp/ib_send_lat_client.txt"
...
function check_roce_network_connection_between_2nodes_same_device
{
    ...
    do
        ...
        client_cmd="ib_send_lat -d ${client_mlx_all[$i]} -x ${GID} -n 5 ${server_host_ip}"

        ssh_exec $server "$server_cmd > $server_result_file 2>&1 &"
        sleep $ssh_interval

        result2=$(ssh_exec $client "$client_cmd" 2>&1)
        client_exit_code=$?

        ssh_exec $server "pkill -9 ib_send_lat" 2>/dev/null
        ssh_exec $client "pkill -9 ib_send_lat" 2>/dev/null

        result1=$(ssh_exec $server "cat $server_result_file 2>/dev/null")
        ssh_exec $server "rm -f $server_result_file" 2>/dev/null

        if [[ "$result2" == *"#bytes"* ]] && [[ "$result2" == *"iterations"* ]] && \
           [[ "$result1" == *"#bytes"* ]] && [[ "$result1" == *"iterations"* ]]; then
            echo "SUCCESS! RoCE network connection between Server-${node1}-${server_eth_all[$i]} and Client-${node2}-${client_eth_all[$i]} Link Yes!"
            echo "Latency test completed successfully"
        else
            echo "FAILED! RoCE network connection between Server-${node1}-${server_eth_all[$i]} and Client-${node2}-${client_eth_all[$i]} Link No!"
            echo "Client exit code: $client_exit_code"
            if [[ "$result1" == *"Waiting for client to connect"* ]]; then
                echo "Server was waiting but client never connected"
            fi
            if [[ -n "$result2" ]]; then
                echo "Client output (last few lines):"
                echo "$result2" | tail -5
            fi
            if [[ -n "$result1" ]]; then
                echo "Server output (last few lines):"
                echo "$result1" | tail -5
            fi
        fi
    done
...
check_roce_network_connection_between_2nodes_same_device
end_time=$(date +%s.%N)
