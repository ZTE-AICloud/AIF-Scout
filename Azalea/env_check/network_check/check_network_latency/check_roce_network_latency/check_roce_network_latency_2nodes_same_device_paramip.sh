...
function check_roce_network_latency_between_2nodes_same_device
{
    ...
    if [ ! -z "$NETWORK_DEVICES" ]; then
        ...
        do
            ...
            server_result=$(ssh_exec $server ifconfig ${server_eth_all[$m]})
            server_eth_ip=$(echo "$server_result" | sed -n "2p" | grep inet | awk '{print $2}')
            if [ -z "$server_eth_ip" ]; then
                server_network_error_msg="${server_network_error_msg} not conf ip"
            fi
            ...
            client_result=$(ssh_exec $client ifconfig ${client_eth_all[$m]})
            client_eth_ip=$(echo "$client_result" | sed -n "2p" | grep inet | awk '{print $2}')
            if [ -z "$client_eth_ip" ]; then
                client_network_error_msg="${client_network_error_msg} not conf ip"
            fi
        ...
        if [[ "${#server_mlx_all[@]}" == "$network_devices_num" ]] && [[ "${#client_mlx_all[@]}" == "$network_devices_num" ]] && [[ "${#server_eth_ips[@]}" == "$network_devices_num" ]] && [[ "${#client_eth_ips[@]}" == "$network_devices_num" ]]; then
        ...
        # Get ip for eth (参数面IP)
        server_eth_ips=()
        client_eth_ips=()
        for ((n=0; n<$server_ibdev_active_num; n++));
        do
            server_result=$(ssh_exec $server ifconfig ${server_eth_all[$n]})
            server_result=$(echo "$server_result" | sed -n "2p" | grep inet | awk '{print $2}')
            client_result=$(ssh_exec $client ifconfig ${client_eth_all[$n]})
            client_result=$(echo "$client_result" | sed -n "2p" | grep inet | awk '{print $2}')
            server_eth_ips+=($server_result)
            client_eth_ips+=($client_result)
        done
    fi
    ...
    do
        ...
        # 使用参数面IP作为目标地址
        cmd2="$IB_TOOL -d ${client_mlx_all[$i]} -x ${GID} -s ${MESSAGE_SIZE} ${IB_ARGS} ${server_eth_ips[$i]}"
        ...
        if [[ "$IB_TOOL" == "ib_send_lat" ]]; then
            if [[ "$result2" == *"#bytes"* ]] && [[ "$result2" == *"iterations"* ]]; then
                ...
                echo "Connection: Server-${server_mlx_all[$i]}-${server_eth_ips[$i]} Client-${client_mlx_all[$i]}-${client_eth_ips[$i]}"
            else
                echo "FAILED! RoCE network latency between Server-${node1}-${server_eth_all[$i]}-${server_mlx_all[$i]} and Client-${node2}-${client_eth_all[$i]}-${client_mlx_all[$i]}: Server-${server_mlx_all[$i]}-${server_eth_ips[$i]} Client-${client_mlx_all[$i]}-${client_eth_ips[$i]}"
                if [[ ! -z "$result2" ]]; then
